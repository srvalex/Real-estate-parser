...
def sync_databases():
    inside_db_path  = os.path.join(_ROOT, "Streamlit Interface", "real_estate.db")
    outside_db_path = os.path.join(_ROOT, "real_estate.db")
    ...
    try:
        ...
        outside_cursor.execute("SELECT * FROM listings WHERE is_available = 1")
...
def sync_from_firestore():
    ...
    db_path        = os.path.join(_ROOT, "Streamlit Interface", "real_estate.db")
    parent_db_path = os.path.join(_ROOT, "real_estate.db")

    print("📥 Fetching available listings from Firestore 'listings' collection...")
    docs = db.collection('listings').where(filter=FieldFilter('is_available', '==', 1)).stream()
